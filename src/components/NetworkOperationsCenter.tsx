...
export default function NetworkOperationsCenter() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [nextPresence, nextTransmissions, nextOverview] = await Promise.all([
      ...
      console.error("NOC synchronization error:", error);
      setStatus("DEGRADED");
    }
  };
  ...
  useEffect(() => {
    ...
    const unsubscribePresence =
      networkOperationsService.subscribePresence(() => void load());
    const unsubscribeTransmissions =
      networkOperationsService.subscribeTransmissions((item) => {
  ...
  return (
    <div className="noc">
      <header className="noc-header">
        ...
        <div className={`noc-live-state state-${status.toLowerCase()}`}>
          <i />
          <div>
      ...
      <section className="noc-kpi-grid">
        <article>
          ...
          <small>Last five minutes</small>
        </article>
        <article>
          <span>TOTAL OBSERVERS</span>
          <strong>{overview.total_observers}</strong>
        ...
        <article className={`threat-${threatLevel.toLowerCase()}`}>
          <span>THREAT LEVEL</span>
          <strong>{threatLevel}</strong>
          <small>{overview.open_transmissions} active transmissions</small>
        </article>
      </section>
      ...
      <div className="noc-main-grid">
        <section className="noc-map-panel">
          <header>
            <div>
              <small>GLOBAL RELAY MATRIX</small>
              <h2>Observer Network</h2>
            </div>
          ...
          <div className="noc-world-map">
            <div className="noc-grid-lines" />
            <div className="noc-continent c-one" />
            <div className="noc-continent c-two" />
            <div className="noc-continent c-three" />
            <div className="noc-continent c-four" />

            <svg viewBox="0 0 100 70" preserveAspectRatio="none" aria-hidden="true">
              <path d="M18 36 C30 10, 38 58, 47 31" />
              <path d="M47 31 C60 12, 67 52, 72 34" />
              <path d="M72 34 C78 46, 79 56, 84 62" />
              <path d="M29 54 C44 70, 50 57, 58 48" />
            </svg>

            {mapNodes.map((node, index) => (
              <div
                key={node.label}
                className={`noc-map-node node-${index + 1}`}
            ...
            <div className="noc-radar-sweep" />
            <div className="noc-map-caption">
              <span>ENCRYPTED ROUTES</span>
              <b>{Math.max(6, overview.active_observers * 2)} RELAYS</b>
            </div>
          </div>

          <div className="noc-region-strip">
            {["NORTH AMERICA", "EUROPE", "ASIA PACIFIC", "DARK RELAY"].map(
              (region, index) => (
                <article key={region}>
                  <i />
                  <div>
        ...
        <aside className="noc-side-stack">
          <section className="noc-presence-panel">
            <header>
              <small>LIVE PRESENCE</small>
              <span>{presence.length} ONLINE</span>
            </header>

            <div className="noc-presence-list">
              {presence.length ? (
                presence.map((item) => (
                  <article key={item.observer_id}>
                    <i />
                    <div>
                      <strong>{item.observer_code}</strong>
                      <small>{item.current_app.toUpperCase()}</small>
                    </div>
                    <span>{timeAgo(item.last_seen)}</span>
                  </article>
                ))
              ) : (
                <p className="noc-empty">Awaiting observer heartbeats...</p>
              )}
            </div>
          </section>

          <section className="noc-system-panel">
            <header>
              <small>SYSTEM TELEMETRY</small>
              <span>NOMINAL</span>
            </header>
            {[
              ["DATABASE", 96],
              ...
              <div className="noc-meter" key={String(label)}>
                <span>{label}</span>
      ...
      <section className="noc-transmission-panel">
        <header>
          <div>
            <small>INCOMING TRANSMISSIONS</small>
            <h2>Global Event Feed</h2>
          </div>
          <span>LIVE</span>
        </header>

        <div className="noc-transmission-grid">
          {transmissions.length ? (
            ...
            <div className="noc-empty">No active transmissions.</div>
          )}
        </div>
      </section>
      ...
      {selectedTransmission && (
        <div
          className="noc-transmission-modal"
          onClick={() => setSelectedTransmission(null)}
        >
          <article onClick={(event) => event.stopPropagation()}>
            <small>PRIORITY TRANSMISSION</small>
            <h2>{selectedTransmission.title}</h2>
            <p>{selectedTransmission.message}</p>
            <dl>
              <div>
                <dt>Origin</dt>
                <dd>{selectedTransmission.origin}</dd>
              </div>
              <div>
                <dt>Severity</dt>
                <dd>{selectedTransmission.severity}</dd>
              </div>
              <div>
                <dt>Decrypt progress</dt>
                <dd>{selectedTransmission.progress}%</dd>
              </div>
            </dl>
            <button type="button" onClick={() => setSelectedTransmission(null)}>
              CLOSE CHANNEL
            </button>
          </article>
        </div>
